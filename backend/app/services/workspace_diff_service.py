...
class WorkspaceDiffService:
    ...
    def build_diff_for_write(self, mgr, session_id: str, path: str, after_bytes: bytes) -> Optional[DiffArtifact]:
        ...
        if not normalized_path or not self.is_trackable(normalized_path):
            card_log('Card', f"[DEBUG P4] skip prewrite diff: path={path!r} normalized={normalized_path!r} trackable={self.is_trackable(normalized_path) if normalized_path else False}")
            return None

        after_text = _decode_text(after_bytes)
        before_text = self._read_current_text(mgr, session_id, normalized_path)
        card_log('Card', f"[DEBUG P4] prewrite diff check: path={normalized_path} before_len={len(before_text) if before_text is not None else 'None'} after_len={len(after_text)}")
        if before_text is None or before_text == after_text:
            reason = 'missing-before' if before_text is None else 'no-change'
            card_log('Card', f"[DEBUG P4] prewrite diff skipped: path={normalized_path} reason={reason}")
            return None
        artifact = _build_diff_artifact(normalized_path, before_text, after_text)
        card_log('Card', f"[DEBUG P4] prewrite diff built: path={normalized_path} +{artifact.additions} -{artifact.deletions} diff_len={len(artifact.diff_text)}")
        return artifact

    def store_snapshot_for_write(self, mgr, session_id: str, path: str, content_bytes: bytes):
        normalized_path = _normalize_workspace_path(path)
        if not normalized_path or not self.is_trackable(normalized_path):
            card_log('Card', f"[DEBUG P4] snapshot skip: path={path!r} normalized={normalized_path!r}")
            return
        card_log('Card', f"[DEBUG P4] snapshot store after manual write: path={normalized_path} bytes={len(content_bytes)}")
    ...
    def capture_diff_after_write_event(self, mgr, session_id: str, path: str) -> Optional[DiffArtifact]:
        normalized_path = _normalize_workspace_path(path)
        if not normalized_path or not self.is_trackable(normalized_path):
            card_log('Card', f"[DEBUG P4] event diff skip: path={path!r} normalized={normalized_path!r}")
            return None

        current_text = self._read_current_text(mgr, session_id, normalized_path)
        if current_text is None:
            card_log('Card', f"[DEBUG P4] event diff skipped: path={normalized_path} current_text=None")
            return None

        latest_text = self._read_snapshot(mgr, session_id, normalized_path, 'latest')
        card_log('Card', f"[DEBUG P4] event diff check: path={normalized_path} latest_len={len(latest_text) if latest_text is not None else 'None'} current_len={len(current_text)}")
        if latest_text == current_text:
            card_log('Card', f"[DEBUG P4] event diff skipped: path={normalized_path} reason=no-change-vs-latest")
            return None

        self._store_snapshot(mgr, session_id, normalized_path, current_text)
        if latest_text is None:
            card_log('Card', f"[DEBUG P4] event diff skipped: path={normalized_path} reason=first-snapshot")
            return None
        artifact = _build_diff_artifact(normalized_path, latest_text, current_text)
        card_log('Card', f"[DEBUG P4] event diff built: path={normalized_path} +{artifact.additions} -{artifact.deletions} diff_len={len(artifact.diff_text)}")
        return artifact
    ...
    def _store_snapshot(self, mgr, session_id: str, path: str, content: str):
        ...
        if latest_text == content:
            card_log('Card', f"[DEBUG P4] snapshot unchanged: path={path}")
            return

        entry_dir = self._entry_dir(path)
        latest_path = self._snapshot_path(path, 'latest')
        previous_path = self._snapshot_path(path, 'previous')
        card_log('Card', f"[DEBUG P4] snapshot write: path={path} entry_dir={entry_dir} had_latest={latest_text is not None}")
