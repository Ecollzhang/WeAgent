...
BUILTIN_TOOL_DEFINITIONS = [
    ...
    _definition(
        ...
        input_schema=_schema(
            {
                ...
                "document_ids": {"type": "array", "items": {"type": "string"}, "default": None},
            },
            ["query"],
        ),
        ...
        markdown=_markdown(
            ...
            "调用 `rag_search`，传入 `query` 和可选的 `domain`（领域过滤）、`workspace_id`（工作空间过滤）、`document_ids`（按文档ID列表过滤）、`top_k`（返回数量）、`score_threshold`（最低相似度阈值）。",
    ...
    _definition(
        ...
        icon="el-icon-menu",
        color="#8b5cf6",
        description="列出所有可用的微服务及其地址，供 Agent 发现和调用",
        ...
        input_schema=_schema({}),
        output_schema=_schema({
            "services": {"type": "object", "description": "服务名称到 base_url 的映射"},
        }),
        markdown=_markdown(
            "列出微服务",
            "当 Agent 需要了解有哪些后端微服务可用时调用。返回服务注册表中的所有服务及其访问地址。",
            "调用 `list_services`，无需参数。返回 `services` 对象，key 为服务名（rag/rd/edu/office），value 为服务 base_url。",
            "服务注册表通过 SERVICE_REGISTRY 环境变量注入容器，只读。",
            '{"name":"list_services","args":{}}',
        ),
    ),
    _definition(
        value="call_service_api",
        name="调用服务API",
        category="tool_service",
        icon="el-icon-connection",
        color="#8b5cf6",
        description="通用微服务 API 调用工具，Agent 可借此调用任何已注册服务的接口",
        handler="service.call",
        tool_names=["call_service_api"],
        permissions={"required": ["network"], "optional": []},
        status="implemented",
        input_schema=_schema(
            {
                ...
                "query_params": {"type": "object", "default": None},
            },
            ["service_name", "path"],
        ),
        output_schema=_schema({
        ...
        markdown=_markdown(
            "调用服务API",
            "当 Agent 需要调用后端微服务的具体接口时使用。先通过 `list_services` 查看可用服务，再通过访问 `/api/{service}/spec` 了解各服务的接口列表，最后用本工具发起实际调用。",
            "调用 `call_service_api`，必填 `service_name`（如 rag/rd/edu/office）和 `path`（如 /api/rag/search），可选 `method`（默认GET）、`body`（POST/PUT的请求体）、`query_params`（URL查询参数）。",
            "只能调用 SERVICE_REGISTRY 中注册的服务；请求自动携带 X-Internal-API-Key 认证头；超时30秒；每个服务都有 `/api/spec` 端点可查询接口列表。",
            '{"name":"call_service_api","args":{"service_name":"rag","method":"POST","path":"/api/rag/search","body":{"query":"微服务架构","top_k":5}}}',
        ),
    ),
]
