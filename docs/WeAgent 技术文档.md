...
WeAgent 是一个面向 AI 协作任务的多端 Agent 工作台。它把自然语言任务、多个可配置 Agent、工具集能力、运行产物和服务预览放在同一个会话空间中，让用户可以像聊天一样发起任务，同时看到任务如何被拆解、执行，并沉淀为文件、页面、工作流或可预览服务。

从技术实现看，WeAgent 以 Flask 后端为协调中心，以 Docker Sandbox 为执行边界，以 Agent / Capability / Artifact 为核心抽象。Web、Electron Desktop 和 Capacitor Android 客户端负责用户交互；后端负责会话、消息、Agent 调度、能力投影和实时推送；Sandbox 内运行 AgentRuntime、Provider Runner、Tool / MCP runtime 和 `weagent-service`，并通过 Socket\.IO 将执行进度、结构化产物和服务预览链接回传到客户端。

本文面向比赛评审、开发维护者和希望理解系统可信度的高级用户，重点说明 WeAgent 的技术结构、模块边界、关键链路、开发者运行方式和已知限制。
...
下图是weagent的系统架构图：用户只面对客户端；后端负责协调和持久化；每个会话对应一个 Sandbox；Sandbox 内部完成 Agent 执行、工具调用、产物生成和服务预览。

![Image](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=YWY3MGUxMDA5OTMxYjhmZGYyY2EyNmQyYjBlYmIxMjRfZTY5ZjgwN2RkMTM5Y2FlNmMxYWFmNDMwN2ZiZThjNDlfSUQ6NzY0OTMyNTk1MjA3NDAyNTk3MF8xNzgxMDE1NzUxOjE3ODExMDIxNTFfVjM)
...
在这义部分我们将从各个层级和代码路径的角度，展示如果想深入挖掘weagent需要从哪些角度来进行。
...
![Image](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=NmJhZmNmMjdhNmNlODEwMDg4NTcwNGE2YWZhNDQ4MzJfYmFhMjc0N2E4ZTU2NmZkMzA0MzE3NjBkMDMxYzRjMjdfSUQ6NzY0OTMyNjE2MjEzMjg5Njc0N18xNzgxMDE1NzUxOjE3ODExMDIxNTFfVjM)
...
![Image](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=N2NmZWZjMGY5ZjRmYTllN2RjYTMxMTI3OWY4YWMzM2RfZmZhMjMyMjIxODJkNTQyZTczZjM4YTllODc2NTUyNjZfSUQ6NzY0OTMzMTYwMzYzNDQ3NDIxNl8xNzgxMDE1NzUxOjE3ODExMDIxNTFfVjM)
...
![Image](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=MDJmNjk5NGFhYTRmNTZkNTMyNGI4MTNkMTI3ZmFmMWZfM2M2OTU4Mzg5MjM2OWU0ZTAyZTMyY2JjZDIyMDYxNGNfSUQ6NzY0OTMyNjI5MDI2MDcwODI5N18xNzgxMDE1NzUxOjE3ODExMDIxNTFfVjM)
...
![Image](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=YzAxMGFmZTYzZTY5YmEwMDUyODc5YmQ1YzcwMzllNTVfZGNjMzRkOTU1ODNlNzU4YjJjNzNmNTgzMDEyNTExNGJfSUQ6NzY0OTMyNjQxMDE5Njg4MDM0NF8xNzgxMDE1NzUxOjE3ODExMDIxNTFfVjM)
...
![Image](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=OTY1Y2EwMzViNTc0YmI4MTA5MGY1MzNhNjAyMDNkYmJfM2Y3NDkwMzQ2MjQ3YzkwNjAyMGY3YWJjY2MzYmZiMmVfSUQ6NzY0OTMyNjU0NjEwNDI1NzQ4MV8xNzgxMDE1NzUxOjE3ODExMDIxNTFfVjM)
