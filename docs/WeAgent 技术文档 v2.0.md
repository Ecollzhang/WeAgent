...
从技术实现看，WeAgent 以 Flask 后端为协调中心，以 Docker Sandbox 作为隔离执行边界，以 Agent、能力（Capability）和产物（Artifact）为核心抽象。用户通过 Web、桌面端或 Android 客户端发起任务；后端创建会话、调度 Agent、管理能力投影；容器内运行 Agent、工具协议、外部执行引擎和服务管理逻辑，再把执行进度、结构化产物和服务链接实时推送回客户端。
...
- 隔离执行：Agent 任务在 Docker 容器内运行，后端负责容器管理、文件代理和实时事件回写。
...
v2.0 调整的是阅读顺序和表达方式，不改变代码与已确认实现所定义的技术边界。
...
| 桌面端 | Electron 29、Vite 5、Vue 2.7、Element UI、electron-builder | 桌面应用外壳，连接外部后端。 |
| Android | Capacitor 6、Vite 5、Vue 2.7 | 移动端主流程，覆盖登录、会话、Agent 和设置等能力。 |
...
- Flask：提供 HTTP API。
- Flask-SocketIO：提供实时事件推送。
- SQLAlchemy：管理数据模型。
- Flask-JWT-Extended：处理用户身份认证。
- Flask-Migrate：支持数据库迁移。
- PyMySQL：连接 MySQL。
...
Agent 任务不直接在后端进程内运行，而是在基于 Docker 的隔离执行环境中运行。

主要技术包括：

- Docker Sandbox：每个正式 Agent 会话对应一个容器会话，用于隔离文件、工具、服务端口和执行状态。
- Orchestrator：容器内任务调度器，负责组织 Agent 执行。
- AgentRuntime：单个 Agent 的运行上下文，负责工作目录、提示词、运行器和输出采集。
- Provider Runner：外部执行引擎运行器，用于调用 Claude Code、Codex 或 OpenCode。
- `weagent-report`：容器内结构化进度和产物上报工具。
- `weagent-service`：容器内服务启动、登记和上报工具。
...
| 基础设施 | 用途 |
| --- | --- |
| MySQL 8.0 | 保存用户、模型配置、Agent、会话、消息、产物和能力数据。 |
| Redis | 作为后端辅助基础设施。 |
| Docker Desktop | 运行 `weagent-sandbox:latest` 容器镜像。 |
| Node.js / npm | 构建 Web、桌面端和 Android 客户端。 |
| Android Studio / Gradle | 构建和调试 Android 客户端。 |

## 3. 系统架构

本章说明 WeAgent 各层如何连接。客户端只负责交互，后端负责状态和调度，容器负责隔离执行，外部执行引擎负责完成具体 AI 任务。

### 3.1 总体架构

WeAgent 采用“多端客户端 + 后端服务 + 容器执行环境 + 外部执行引擎”的分层架构。

```mermaid
flowchart TD
    C["Web / Desktop / Android"] -->|REST API / Socket.IO| B["Flask Backend"]
    B -->|Session / Message / Agent / Artifact| D["MySQL / Redis"]
    B -->|Sandbox Host Proxy| S["Docker Sandbox"]
    S -->|AgentRuntime / Tool / MCP / Service| R["Container Runtime"]
    R -->|Provider Runner| P["Claude Code / Codex / OpenCode"]
    R -->|Report / Events / Service Preview| B
```

读图方式：用户入口在客户端，业务状态在后端，真实执行在 Docker Sandbox。能力配置会先进入数据库，再投影到容器内的运行视图；产物和服务事件从容器回到后端，再推送给客户端。
...
隔离执行层是 Docker Sandbox。一个正式 Agent 会话会绑定一个 sandbox session，容器内保存工作区、Agent 运行状态、能力投影、事件日志和服务日志。
...
本章说明系统有哪些主要组成部分。这里使用表格是为了让读者快速看清模块范围，每个小模块都用一句话说明职责。
...
Docker Sandbox 是 WeAgent 的执行边界。后端负责创建和管理容器，容器负责运行 Agent、读写工作区文件、调用工具协议、启动服务，并把事件回传给后端。
...
容器接口不是面向用户的公开 API，而是后端管理容器执行环境的内部控制面。它把容器内正在发生的任务执行、文件变化、工具调用和服务状态转换为后端可以管理的结构化状态。
...
外部执行引擎适配机制负责把 Claude Code、Codex 和 OpenCode 统一成同一种运行协议。Agent 配置中保存适配类型，容器内根据配置选择对应运行器，再把 API Key、Base URL、模型名和工作目录注入执行环境。
...
| 建立执行边界 | 宿主侧管理器创建或恢复会话容器，并把 Agent 配置和能力投影写入容器。 |
...
多 Agent 协作的核心是主持 Agent 的任务拆解和执行调度，而不是多个 Agent 同时输出聊天内容。

当会话包含多个 Agent 且用户没有指定目标 Agent 时，后端会先让主持 Agent 生成任务计划。计划中可以包含任务节点、依赖关系和并行分组。后端再根据计划把子任务发给 worker Agent 执行。

工作流图是多 Agent 协作的可视化入口。它作为 `workflow` 消息元素展示，包含 `nodes`、`edges`、`parallel_groups` 和来源信息。用户可以从消息中预览工作流，在会话面板中编辑节点和连线，也可以把选中的工作流作为下一轮任务的调度上下文。

工作流图的边界需要明确：它不是全局 Agent 配置，也不是独立的后端工作流模型。它先作为消息元素进入前端，再由会话 UI 保存和复用。发送消息时，客户端把选中的工作流放入 `workflow` payload，后端把它写入本轮主持 Agent 的上下文。
...
本章沿用源文档关键数据流的方向，用简洁时序图说明系统如何运行。每张图控制在 5 到 6 个参与者以内，复杂链路拆成短图，避免在飞书中难以阅读。

### 6.1 用户消息到 Agent 响应

这条链路说明一条用户消息如何进入后端、触发容器执行，并通过实时事件回到客户端。
...
关键点是后端先保存状态，再进入容器执行。前端收到的进度来自 Socket.IO，而不是等待一次 HTTP 请求完成。

### 6.2 多 Agent 协作与工作流

这条链路说明主持 Agent 如何生成计划，并把子任务分配给其他 Agent。
...
工作流图既是展示结果，也是下一轮调度上下文。它不修改全局 Agent，只影响当前会话中的任务分配。

### 6.3 能力投影到执行环境

这条链路说明能力如何从配置进入容器，而不是让 Agent 直接访问全局能力库。
...
能力投影把全局能力库和当前会话运行环境分开。这样可以审查导入、限制权限，并记录调用行为。

### 6.4 产物生成、展示与编辑

这条链路说明 Agent 输出如何变成消息元素，并进入工作台继续编辑。
...
消息卡片适合快速查看，工作台适合继续加工。两者共享同一套消息元素和文件 API。

### 6.5 文件迁移

这条链路说明用户如何把一个会话中的工作区文件迁移到另一个会话。
...
迁移分为 preview 和 migrate 两步。preview 不复制文件，只让用户确认路径映射、冲突和跳过项。

### 6.6 服务启动、代理与预览

这条链路说明容器内服务如何变成用户可打开的预览链接。
...
用户打开的是后端代理地址，不是容器内 `127.0.0.1:<port>`。代理层负责 token、session 和 service 校验。

### 6.7 多端连接后端

Web、桌面端和 Android 都连接同一个后端。桌面端和 Android 首次启动时需要配置 `serverUrl`；真机访问电脑后端时不能填写手机自身的 `localhost`，应使用电脑局域网 IP 或公网地址。
...
本轮 v2.0 文档重组没有重新运行完整 smoke，完整验证状态为 `待确认`。当前可以按以下路径验证：
...
这些接口共同支撑登录、会话、Agent 执行、产物展示、能力管理和容器代理。
...
关键模型关系可以概括为：用户拥有模型配置和会话；会话包含参与者、消息和容器状态；消息连接 Agent 执行记录和产物元素；能力通过绑定关系进入 Agent，再投影到容器。
...
本章保留严谨性，说明当前文档和系统能力的边界。

| 项目 | 状态 | 说明 |
| --- | --- | --- |
| 完整 smoke | 待确认 | 本轮 v2.0 文档重组没有重新运行完整后端、前端、桌面端或 Docker smoke。 |
...
本章按症状定位常见问题。排查时先确认后端健康状态，再看容器、外部执行引擎和前端实时连接。
...
| Docker Sandbox（隔离执行环境） | 每个会话对应的 Docker 容器，用于隔离工作区、工具、服务端口和 Agent 执行状态。 |
