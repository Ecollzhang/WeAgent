...
当前文档的重点是说明真实技术结构、模块边界、关键链路、开发者运行方式和已知限制。文档按模块使用已确认事实源：Toolset / Capability 以 `origin/combine/toolset_v1.1.0` 为依据；其余客户端、后端、Sandbox、Artifact / Workbench、服务预览和实时事件以 `origin/feature/user_manual_and_product_introduction_v1.1.1` 为依据。不能从代码或分支确认的内容不写入本文正文。
...
| `origin/feature/user_manual_and_product_introduction_v1.1.1` | `403bb1a` / 2026-06-07 19:25 | 作为 Toolset 以外模块的根本依据，包括客户端、多端、Agent 管理、会话收藏、Artifact / Workbench、服务预览和实时事件。 |
...
| 层级 | 主要职责 | 技术与路径 | 事实依据 |
|---|---|---|---|
| 客户端层 | 提供聊天、Agent 管理、会话收藏、工具集、设置、多端连接 UI。 | Web：`frontend/`；Desktop：`clients/desktop/`；Android：`clients/android/` | `origin/feature/user_manual_and_product_introduction_v1.1.1` |
...
| 产物层 | 把 Agent 输出转换为消息元素、Artifact、服务卡片、Workbench、Diff 和文件迁移能力。 | `artifact_controller.py`、`message_element_builder.py`、`sandbox_event_bridge.py`、`ArtifactWorkbench` | `origin/feature/user_manual_and_product_introduction_v1.1.1` |
...
Agent 管理页面的源码事实是分类、卡片、拖拽移动、创建 / 编辑和系统 Agent 只读查看；会话收藏和搜索属于 Favorites 页面，不属于 Agent 管理页面。
...
| 会话收藏搜索 | Favorites 页面按标题、参与者、最近消息和类型过滤 | `frontend/src/views/Favorites.vue`，`clients/desktop/src/views/Favorites.vue` |
...
产物层负责把 Agent 输出转成用户能查看、复用和继续编辑的结构化结果。根据 `origin/feature/user_manual_and_product_introduction_v1.1.1`，该层包括基础 Artifact、消息元素、服务卡片、Artifact Workbench、文件编辑、diff 和文件迁移。

实现范围：

- 包含：`Artifact` 模型、消息元素、代码 / 表格 / 图片 / 文件 / service / diff 类型、Artifact Workbench、文件编辑、diff、文件迁移、服务类产物预览。
...
结论：客户端与多端连接模块负责把同一个 WeAgent 后端投射到 Web、Desktop 和 Android 客户端。

实现范围：

- 包含：Web 工作台、Electron Desktop、Capacitor Android、`serverUrl` 配置、REST API client、Socket.IO client。
...
- 包含：Artifact 数据模型、消息元素适配、基础产物展示、service 卡片、Artifact Workbench、代码编辑、HTML 页面编辑、图片裁剪、Diff 和文件迁移。
- 不包含：没有消息元素或 workspace 来源的外部资产管理。
- 上游：Provider 输出、`weagent-report`、ServiceManager、Sandbox events。
- 下游：前端 MessageBubble、Artifact API、ArtifactWorkbench、DiffViewCard、FileMigrationDialog。
...
| 文件可迁移到其他会话 | Dashboard 引入 `FileMigrationDialog` 并打开迁移对话框 | `frontend/src/views/Dashboard.vue` |
