...
- 本文所列功能已通过浏览器 UAT、服务端审计和自动化回归；验收证据见 0.1。

### 0.1 实施与验收证据

- 自动化回归：后端 `529 passed`；前端 27 份契约与 Vue 编译测试全部通过；前端生产构建通过。
- 教师真实 UAT：课程成员关系自动识别教师身份；仅发送自然语言请求即可生成 8 页
  `paper_annotation` 课件；课件形成不可变 v4，新旧版本可回看；HTML 预览、PPTX/HTML
  导出和 8 页逐页视觉检查通过；自然语言课时修改已持久化为 50 分钟。
- RAG 真实 UAT：上传可解析的真实 PDF 后完成索引并命中 3 个正文片段；同一教师的另一课程
  对同一精确短语返回 0 条，课程级隔离成立。
- 学生真实 UAT：课程成员关系自动识别学生身份，只展示学习规划师、练习教练和笔记整理师；
  伪造教师 Agent 在 bootstrap 阶段返回 403，直接申请教师写工具同样返回 403。
- 聊天与业务页 UAT：课件业务页可返回创建该产物的精确会话，刷新后会话不丢失；聊天卡片可打开
  业务页、认证预览和导出，不再把受保护 URL 直接塞入无认证 iframe。
- 灰度与兼容 UAT：Education 五个灰度键均生效；EDU/RD 卡片按领域隔离；历史 RD 会话在旧沙箱
  不支持热更新授权时会保存工作区并升级到最新镜像，随后仍能读取原项目数据。
