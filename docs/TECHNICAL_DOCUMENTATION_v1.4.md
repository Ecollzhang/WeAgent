...
结论：API Blueprint 按业务域拆分。读者应先看每个域负责什么，再看具体路径。
...
`Conversation` 保存标题、会话类型、owner、sandbox session / container / port / status、last active 等信息。会话收藏字段 `is_favorite` 支撑会话列表、详情页和 Favorites 页的收藏状态。
...
结论：Docker Sandbox 系统是 WeAgent 的执行边界。后端负责创建和管理容器，容器负责运行 Agent、读写工作区文件、调用工具 / MCP，并把服务和事件回传给后端。
...
结论：Artifact 与消息元素系统负责把 Agent 输出从纯文本拆成结构化内容，让前端能按代码、文件、图片、表格、服务、Diff、工作流等类型展示。
...
结论：Workbench 与文件闭环系统负责把消息里的结构化产物变成可查看、可编辑、可比较、可迁移的工作区文件。
