...
本文会保留必要的代码路径，帮助读者定位实现位置；正文不会展开为完整实现索引。开发者审计实现时，以代码、配置、测试和运行脚本作为最终事实来源。

### 1.5 事实边界

正文以当前代码、配置、运行脚本、模型、路由、测试和已确认实现为准。未确认信息以 `待确认` 标记，避免误读为已完成能力。

本文只说明已能从代码、配置、运行脚本、模型、路由、测试或已确认实现中支撑的内容。文档中的架构图和时序图用于帮助理解系统边界，不替代源码、配置和测试结果。

### 1.6 核心术语速览

首次阅读时，可以先对齐下面几个词。第 11 章会给出更完整的术语表。

*这些术语会在架构图、模块说明和时序图中反复出现。*

| 术语 | 先理解成什么 |
| --- | --- |
| Agent | 会话中承担任务的 AI 工作角色。 |
| Docker Sandbox | 每个 Agent 会话对应的容器化沙盒环境，用来隔离文件、工具、端口和运行状态。 |
| sandbox session | 后端用于管理某个沙盒容器生命周期的会话记录。 |
| Provider / Provider Runner | Provider 是 Claude Code、Codex、OpenCode 等外部执行提供方；Provider Runner 是容器内包裹这些 CLI 的适配层。 |
| Capability | Skill、Tool、MCP、Plugin 的统一能力抽象。 |
| Message.elements | 消息中的结构化展示单元，例如文件、代码、图片、服务、diff、workflow。 |
| Artifact / Workbench | Artifact 是可保存的产物记录；Workbench 是查看、编辑、比较和迁移产物的工作台。 |
...
本章说明 WeAgent 各层如何连接。**客户端负责交互，后端负责状态和调度，Docker Sandbox 负责沙盒隔离和任务运行，外部执行引擎负责完成具体 AI 任务。**
...
Web 是主功能基准，包含 Dashboard、会话、Agent 管理、Tools、Favorites、Settings、Artifact Workbench、文件迁移、服务面板和工作流图。桌面端通过 Electron 提供接近 Web 的桌面体验。Android 通过 Capacitor 承载移动端主流程，能力范围与 Web 和桌面端不同。

三个客户端连接同一个后端，但能力覆盖并不完全相同。
...
Android 是移动端主流程版本。服务日志、工作流编辑、文件迁移等高密度工作台能力以 Web / Desktop 为准。
...
本章说明 WeAgent 最重要的工程机制。读者可以把本章当作“系统为什么能跑起来”的核心说明。
...
这里需要区分两个概念：**Provider 是外部执行提供方**，例如 Claude Code、Codex、OpenCode；**Provider Runner 是 WeAgent 在容器内包裹 Provider CLI 的适配层**。Runner 的职责不是决定任务由谁执行，而是封装具体 CLI 的命令格式、环境变量、输出解析、错误识别和停止行为。任务分配发生在后端消息服务和容器内调度器之间。
...
| 会话级 `adapter_name` | 当前会话内的适配器名称快照，只随当前消息 payload 生效。 |
| `ProviderRunnerFactory` | 把 `claude`、`claude_code`、`codex`、`opencode` 映射为具体运行器。 |
| Provider CLI | Claude Code、Codex、OpenCode 等实际执行外部调用的命令行工具。 |
...
读图方式：左侧是用户输入和会话上下文，中间是调度决策，右侧是执行结果和工作流复用。`workflow` 不是全局 Agent 配置，而是当前会话轮次中的调度上下文；用户可以预览、编辑、复制并选择它作为下一轮任务的计划约束。
...
这张图强调两个边界：服务只在 Docker Sandbox 内运行，用户访问入口不是容器端口；后端生成 `proxy_url` 并控制访问边界，前端只展示预览、日志、重启和停止等操作入口。这样 Agent 可以启动本地开发服务，用户仍然通过统一的后端代理访问。
...
本章通过多张时序图说明系统如何运行。**每张图聚焦一个链路，必要时把长链路拆成多张短图。** 为保证在在线文档和 Markdown 中可读，每张图只保留关键角色；更细的函数调用和类调用应回到对应代码路径查看。

*这些图强调关键状态变化，不代表完整代码调用栈。*
...
sequenceDiagram
    ...
    R->>P: 调用外部执行 CLI
    P-->>R: 输出片段和错误事件
    R-->>AR: 归一化 provider 事件
    AR-->>E: 上报执行事件和产物
    E->>D: 更新 Message / AgentRun
    E-->>C: 通过 Socket.IO 推送状态和元素
```

读图说明：

- 后端先保存用户消息和执行记录，再把任务交给容器，这样失败时仍然能追踪任务状态。
- Provider CLI 的输出不会直接返回给前端，而是先由 Provider Runner 归一化，再经 AgentRuntime 和事件桥接写入 `Message` 与 `AgentRun`。
...
sequenceDiagram
    ...
    M->>R: 调用外部执行引擎
    R-->>M: 返回计划文本和结构化 plan
    M-->>O: 返回 nodes / edges / parallel_groups
    O-->>B: 回传 workflow 消息元素
    B->>O: 按计划分发 worker 子任务
    O->>W: 执行子任务
    W->>R: 调用外部执行引擎
    R-->>W: 返回子任务结果
    W-->>O: 回传结果
...
- 主持 Agent 负责生成任务计划，worker Agent 负责执行子任务；两者都由 Orchestrator 调度，并通过 Provider Runner 调用外部执行引擎。
- `workflow` 消息元素保存的是当前轮次计划，不是全局配置。
- 用户可以在前端预览、编辑、复制并选择工作流，让下一轮任务按该计划继续执行。

### 6.3 能力入库与投影到沙盒

这条链路分两步：第一步是能力进入能力库，第二步是会话创建或恢复时把 Agent 可用能力投影到沙盒。核心原则是 Agent 不直接访问全局能力库，而是读取当前会话的能力投影视图。
...
- 用户导入或绑定能力后，Capability API 先保存版本、资产、绑定和审计信息。
- 创建或恢复会话时，Projection Service 把当前 Agent 可用能力写入容器内 `/workspace/.weagent/*`。
...
sequenceDiagram
    participant S as Sandbox
    participant E as Event Bridge
    participant MB as Element Builder
    participant D as Database
    participant UI as MessageBubble
    participant W as Workbench
    participant F as File API

    S-->>E: 上报文件、diff、service 或 workflow
    E->>MB: 转换为 message elements
    MB->>D: 写入 Message.elements / Artifact
    MB-->>UI: 推送结构化消息元素
    UI->>W: 打开产物工作台
    W->>F: 保存编辑或读取文件
    F-->>S: 写回工作区或读取文件内容
...
- 事件桥接接收容器事件，消息元素构建器再把它转换为 `Message.elements` 和 `Artifact`，消息组件按类型展示为不同卡片。
- 工作台不绕过后端直接写容器文件，而是通过 File API 读写工作区。
...
- 迁移完成后，后端会在目标会话创建迁移结果消息，并通过实时通道提示用户哪些文件已进入新会话。
...
sequenceDiagram
    ...
    A->>WS: start port / command / cwd
    WS->>SM: 启动进程并记录 service_id
    SM-->>WS: 返回状态、日志和端口
    WS-->>B: 上报 service 元数据
    B-->>SC: 生成带 proxy_url 的服务卡片
    C->>SC: 打开预览或查看日志
    SC->>B: 访问 proxy_url
    B->>SM: 代理到容器内服务端口
```
...
- Agent 通过 `weagent-service start` 启动容器内服务，`ServiceManager` 负责登记 `service_id`、端口和日志。
- 后端代理为服务生成带 token 的 `proxy_url`，前端用服务卡片展示预览、日志、重启和停止入口。
...
sequenceDiagram
    ...
    C->>Setup: 未配置时进入后端地址配置
    Setup->>L: 保存 serverUrl
...
这些接口共同支撑登录、会话、Agent 执行、产物展示、能力管理和容器代理。表格按业务域组织，而不是按代码目录或 URL 字母顺序组织；开发者可以先根据能力类型找到接口域，再回到对应 controller 或 service 查看字段级细节。
...
本章说明当前系统能力的边界。这里列出的内容不是产品缺陷清单，而是开发、演示和部署时需要提前知道的约束：哪些能力有端侧差异，哪些能力依赖本地环境，哪些验证还需要重新执行后才能声明通过。

| 项目 | 状态 | 说明 |
| --- | --- | --- |
| 完整 smoke | 待确认 | 当前未提供完整后端、前端、桌面端或 Docker smoke 的重新验证记录。 |
| Android 客户端 | 端侧差异 | Android 当前按移动端主流程实现，能力范围不等同于 Web 或桌面端。 |
...
本章按症状定位常见问题。推荐排查顺序是：先确认后端是否健康，再确认容器是否运行、外部执行引擎是否可用、实时连接是否收到事件，最后检查前端是否正确展示。表格中的处理方式是第一轮定位建议，不替代完整日志分析。
...
| Provider Runner（执行引擎运行器） | WeAgent 在容器内包裹 Provider CLI 的适配层，负责统一命令、事件和错误处理。 |
| Docker Sandbox（沙盒隔离环境） | 每个会话对应的 Docker 容器，用于隔离工作区、工具、服务端口和 Agent 执行状态。 |
| sandbox session | 后端用于管理某个沙盒容器生命周期的会话记录，包含容器、端口和运行状态。 |
| Sandbox API | 后端访问容器内 Agent、文件、工具、MCP、服务和事件的内部接口。 |
| Orchestrator（任务调度器） | 容器内组织 Agent 执行、主持 Agent 计划和 worker 子任务的调度组件。 |
| AgentRun | 记录单个 Agent 在一轮会话中的执行状态、错误、开始时间和结束时间。 |
| SandboxEventBridge | 把容器事件转换成消息状态、产物元素和 Socket.IO 推送的后端桥接组件。 |
...
| Message.elements | 消息中的结构化展示单元，例如代码、文件、图片、服务、diff 和 workflow。 |
