<template>
  <div class="slide-editor" data-testid="slide-document-editor">
    <aside class="slide-list">
      ...
      <div class="theme-selector" data-testid="presentation-theme-selector">
        <span>课件风格</span>
        <el-select
          v-model="draft.theme.style"
          size="small"
          style="width: 100%"
          @change="markChanged"
        >
          <el-option
            v-for="(theme, code) in presentationThemes"
            :key="code"
            :label="theme.label"
            :value="code"
          >
            <span class="theme-option-dot" :style="{ background: theme.accent }"></span>
            <b>{{ theme.label }}</b>
          </el-option>
        </el-select>
      </div>
      <div class="slide-list-actions">
        <el-button size="mini" icon="el-icon-plus" @click="addSlide">添加页面</el-button>
      </div>
      <button
...
function editorDraft(value) {
  ...
  draft.theme.style = normalizePresentationTheme(draft.theme).style
...
export default {
  ...
  computed: {
    ...
    themeVariables() {
      const theme = normalizePresentationTheme(this.draft.theme)
      return {
        '--slide-background': theme.surface,
  ...
  methods: {
    ...
    addSlide() {
      ...
      this.draft.slides.splice(index, 0, {
        id: `teacher-slide-${Date.now()}`,
        title: '新页面',
        layout: 'content',
        blocks: [{
          type: 'text',
    ...
    removeSlide() {
      if (this.draft.slides.length <= 1) return
      this.draft.slides.splice(this.selectedIndex, 1)
      this.selectedIndex = Math.min(this.selectedIndex, this.draft.slides.length - 1)
      this.markChanged()
    },
    exportDocument() {
...
.document-title span, .theme-selector > span { display: block; margin-bottom: 7px; color: #607570; font-size: 12px; font-weight: 700; }
.theme-selector { margin-bottom: 12px; }
.theme-option-dot { display: inline-block; width: 9px; height: 9px; margin-right: 8px; border-radius: 50%; }
.slide-list-actions { margin-bottom: 14px; }
...
.slide-stage { margin: 24px; padding: 30px 34px; border: 1px solid color-mix(in srgb, var(--slide-primary) 22%, transparent); border-radius: 16px; color: var(--slide-text); background: var(--slide-background); box-shadow: 0 18px 45px rgba(40, 82, 75, .08); overflow-y: auto; transition: background .2s ease, color .2s ease; }
.slide-stage[data-theme="paper_annotation"] { border-radius: 3px; box-shadow: 8px 10px 0 rgba(76, 61, 42, .08); }
.slide-stage[data-theme="storybook"] { border-radius: 34px; }
.slide-stage[data-theme="dark_focus"] { box-shadow: 0 20px 48px rgba(0, 0, 0, .28); }
.slide-stage header { display: grid; grid-template-columns: 92px minmax(0, 1fr) auto; align-items: center; gap: 14px; margin-bottom: 22px; }
.slide-stage header span { color: var(--slide-primary); font-size: 10px; font-weight: 900; letter-spacing: .12em; }
.block-grid { display: grid; gap: 12px; }
.block-grid article { padding: 14px 16px; border: 1px solid color-mix(in srgb, var(--slide-primary) 17%, transparent); border-radius: 11px; background: color-mix(in srgb, var(--slide-background) 94%, var(--slide-primary)); }
...
.structured-preview { max-height: 110px; overflow: auto; color: var(--slide-text); font-size: 13px; line-height: 1.65; white-space: pre-wrap; }
.speaker-notes { margin-top: 20px; padding-top: 18px; border-top: 1px dashed #cbdad6; }
.speaker-notes label { display: block; margin-bottom: 8px; color: var(--slide-muted); font-size: 12px; font-weight: 700; }
